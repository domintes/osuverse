...
export default function SearchClient() {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            ...
            const res = await fetch(`/api/search?${queryParams.toString()}`, { headers });
    ...
    // Prefill from URL query params and auto-search when token is ready
    useEffect(() => {
        if (!searchParams) return;
        const qpQuery = searchParams.get('query') || '';
        const qpArtist = searchParams.get('artist') || '';
        const qpMapper = searchParams.get('mapper') || '';
        const qpStatus = searchParams.get('status') || undefined;
        const qpMode = searchParams.get('mode') || undefined;

        // Set fields only if present to avoid overriding user typing
        if (qpQuery) setQuery(qpQuery);
        if (qpArtist) setArtist(qpArtist);
        if (qpMapper) setMapper(qpMapper);
        if (qpStatus || qpMode) {
            setFilters(prev => ({
                status: qpStatus || prev.status,
                mode: qpMode || prev.mode
            }));
        }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [searchParams]);

    // Trigger search when URL params change and token is available
    useEffect(() => {
        if (!token || !searchParams) return;
        const hasAny = (searchParams.get('query') || searchParams.get('artist') || searchParams.get('mapper'));
        if (hasAny) {
            handleSearch();
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
